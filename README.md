...
- The worker groups raw samples by the device-stamped `local_day`, so late samples repair the
  correct local day through stable upsert keys.
...
1. `load-daily-stats` groups required raw metrics by the device-stamped `local_day`.
2. `classify-and-upsert` derives cycle phase, readiness, and hormone estimates and upserts
...
bun run --cwd mobile gen:api  # API must be running
```
