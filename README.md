...
Clair band or local simulator
  ...
  → PUBLISHED state + worker rollup refresh and classification
...
- Raw inserts carry their `batch_id`; event delivery and derived writes are designed for safe replay.
- Inngest separates the worker into independently retryable steps.
- The worker refreshes affected continuous-aggregate buckets before reading daily statistics, so late
  samples repair affected days through stable upsert keys.
- AI-generated health cards use a 15-day change signature and fall back to deterministic rules when
...
- the nullable `batch_id` used to attribute newly ingested raw samples;
...
2. Prisma records a `SyncBatch` in `RECEIVED` state first. A client-supplied `clientBatchId` makes
   retries reuse the same row, while a stored content hash rejects reuse of that key with different
   samples.
3. Samples are inserted into `raw_biomarker` with that batch's `batch_id`, using chunked `UNNEST`
   writes and `ON CONFLICT DO NOTHING`. The batch advances to `RAW_WRITTEN`, and
   `Device.lastSyncedAt` moves forward when appropriate.
4. The API publishes typed `device/batch.synced` with ID `device-batch-<batchId>` for delivery
   deduplication, then advances the batch to `PUBLISHED`.
5. The API's `recover-stale-batches` cron reconciles stranded work every 10 minutes. It can promote
   a `RECEIVED` batch when its exactly attributed raw rows prove the write committed, and republishes a due `RAW_WRITTEN` batch
   with the same deterministic event ID.

The worker serves functions at `:3311/api/inngest`. `compute-daily-insights` runs four named,
independently retryable steps:

1. `refresh-rollups` refreshes completed hourly and daily buckets after late or newly synced data.
2. `load-daily-stats` reads required metrics from `biomarker_1d`.
3. `classify-and-upsert` derives cycle phase, readiness, and hormone estimates and upserts
   `DailyInsight` by `(userId, date)`.
4. `mark-batch-processed` advances the source batch to `PROCESSED`.
...
- `SyncBatch` plus raw `batch_id` attribution forms a cross-database state machine: the ledger records
  intent before the time-series write, and reconciliation repairs interruptions between the stores.
...
- Batch state progresses through `RECEIVED`, `RAW_WRITTEN`, `PUBLISHED`, and `PROCESSED`; `FAILED`
  records an ingest failure rather than silently losing it.
