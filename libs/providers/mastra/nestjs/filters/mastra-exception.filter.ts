...
export class MastraExceptionFilter implements ExceptionFilter {
  private readonly logger = new Logger(MastraExceptionFilter.name);
    ...
    // Don't try to send response if headers already sent (e.g., mid-stream)
    ...
    const requestId =
      ...
      (request as FastifyRequest & { mastraRequestId?: string })
        .mastraRequestId;
    ...
    if (normalized.status >= 500) {
      this.logger.error(
        `${request.method} ${path} - ${normalized.status} ${normalized.error}`,
  ...
  private normalizeError(exception: unknown): NormalizedError {
    ...
    if (
      ...
      typeof (exception as { details?: unknown }).details === 'object' &&
      'status' in (exception as { details: object }).details
    ) {
      const details = (exception as { details: { status?: unknown } }).details;
          ...
          error:
            status >= 500
              ...
              : (exception as { message?: string }).message ||
                'An error occurred',
          code:
            (exception as { code?: string }).code || this.getErrorCode(status),
        };
      }
    }
