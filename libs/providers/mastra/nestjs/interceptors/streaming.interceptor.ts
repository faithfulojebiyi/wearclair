import type {
  MCPHttpTransportResult,
  MCPSseTransportResult,
 ...
 *
 * Fastify adaptation: the MastraController uses `@Res()` (non-passthrough), so
 * this interceptor OWNS sending the response for every response type.
 * - JSON responses go through `reply.send()` so Fastify's onSend hooks (CORS,
 *   compression) still run.
 * - Streaming / datastream / MCP responses call `reply.hijack()` and write to
 *   the raw Node `ServerResponse` (`reply.raw`) — the same low-level API the
 *   upstream Express adapter used. NOTE: hijacking bypasses Fastify's onSend
 *   hooks, so CORS headers are not auto-applied to streamed responses (handle
 *   at the proxy / a preHandler hook if browsers must stream cross-origin).
 */
@Injectable()
export class StreamingInterceptor implements NestInterceptor {
    ...
    return next.handle().pipe(
      switchMap((result: unknown) => {
        ...
        switch (type) {
          ...
          case 'json':
            return from(this.handleJson(context, handlerResult.data));
          default:
            return from(this.handleJson(context, result));
        }
      }),
    );
  }
  ...
  private getResultType(
    result: unknown,
  ): RouteHandlerResult['responseType'] | null {
    if (
      result === null ||
  ...
  /**
   * Send a JSON response through Fastify's reply (preserves onSend hooks).
   */
  ...
  /**
   * Handle streaming response - write chunks to the raw response.
   */
  private async handleStreaming(
    ...
    reply.hijack();
    const response: ServerResponse = reply.raw;
    ...
    response.setHeader(
      'Content-Type',
      streamFormat === 'sse' ? 'text/event-stream' : 'text/plain',
    );
    ...
    const hasFullStream =
      data !== null && typeof data === 'object' && 'fullStream' in data;
    const stream = hasFullStream
      ? (data as { fullStream: ReadableStream }).fullStream
      : (data as ReadableStream);
    const reader = stream.getReader();
    const unregisterSse =
      streamFormat === 'sse'
        ? this.shutdownService.registerSseClient(response)
        : null;
   ...
   */
  private async handleDatastream(
    context: ExecutionContext,
    result: RouteHandlerResult,
  ): Promise<void> {
    const reply = context.switchToHttp().getResponse<FastifyReply>();
  ...
  private async handleMcpHttp(
    ...
    } = result.data as MCPHttpTransportResult;
  ...
  private async handleMcpSse(
    ...
    const { server, ssePath, messagePath } =
      result.data as MCPSseTransportResult;
    ...
    try {
      ...
      if (!response.headersSent) {
        ...
        response.setHeader('content-type', 'application/json');
        response.end(
          JSON.stringify({ error: 'Error handling MCP SSE request' }),
        );
      }
    }
  }
