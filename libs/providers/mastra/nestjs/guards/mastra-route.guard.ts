...
export class MastraRouteGuard implements CanActivate {
  constructor(
    ...
    @Inject(MastraThrottleGuard)
    private readonly throttleGuard: MastraThrottleGuard,
  ) {}

  async canActivate(context: ExecutionContext): Promise<boolean> {
    const request = context.switchToHttp().getRequest<FastifyRequest>();
    const method = request.method.toUpperCase();
    const routePath = getMastraRoutePath(
    ...
    if (
      this.options.auth?.enabled !== false &&
      (this.options.auth?.enabled || this.mastra.getServer()?.auth)
    ) {
    ...
    if (this.options.rateLimitOptions?.enabled !== false) {
      const { limit, windowMs } = this.throttleGuard.getRateLimitSettings(
        request,
        undefined,
        routePath,
      );
