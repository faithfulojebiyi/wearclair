...
export class MastraThrottleGuard implements CanActivate, OnModuleDestroy {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const { limit, windowMs } = this.getRateLimitSettings(
      request,
      throttleMetadata,
    );
  ...
  async checkLimit(
    ...
    const path = rateLimitPath ?? getRequestPath(request);
   ...
   *
   * Security note: Fastify's `request.ip` respects the `trustProxy` setting.
   * When `trustProxy` is configured, `request.ip` is the correct client IP from
   * X-Forwarded-For; otherwise it is the socket address, preventing
  ...
  private evictOldestEntries(): void {
    ...
    this.logger.debug(
      `Evicted ${toRemove} oldest rate limit entries due to max size`,
    );
  }
