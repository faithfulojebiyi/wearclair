 ...
 *
 * SSE clients are tracked as Node `ServerResponse` objects — on Fastify the
 * streaming interceptor writes to `reply.raw` (a ServerResponse), so the same
 * low-level write/end/once API the upstream Express adapter used applies here.
 */
@Injectable()
export class ShutdownService implements OnModuleDestroy, OnApplicationShutdown {
   ...
   */
  registerSseClient(response: ServerResponse): () => void {
  ...
  notifySseClients(): void {
    for (const res of this.sseClients) {
      ...
      try {
        ...
        this.logger.warn(
          `Failed to notify SSE client: ${error instanceof Error ? error.message : String(error)}`,
        );
        this.sseClients.delete(res);
      }
    }
  ...
  async onApplicationShutdown(signal?: string): Promise<void> {
    ...
    if (this.activeRequests.size === 0) {
      this.logger.log(
        `Shutdown (${signal}): No active requests, proceeding immediately`,
      );
      return;
    }
    ...
    const timeout = new Promise<void>((resolve) => {
      timer = setTimeout(() => {
        if (this.activeRequests.size > 0) {
          this.logger.warn(
            `Shutdown timeout: ${this.activeRequests.size} requests still active, forcing shutdown`,
          );
          for (const [id, info] of this.activeRequests) {
            const elapsed = Date.now() - info.startTime;
            this.logger.warn(
              `  - ${id}: ${info.path} (running for ${elapsed}ms)`,
            );
          }
        }
        resolve();
