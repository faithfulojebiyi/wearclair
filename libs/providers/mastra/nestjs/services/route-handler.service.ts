...
export interface RouteHandlerResult {
  ...
  responseType:
    'json' | 'stream' | 'datastream-response' | 'mcp-http' | 'mcp-sse';
...
export class RouteHandlerService {
  ...
  matchRoute(method: string, path: string): RouteMatch | null {
    ...
    const methodsToCheck =
      method === 'OPTIONS' ? ['ALL', method] : [method, 'ALL'];
      ...
      if (patternPart.startsWith(':')) {
        ...
        try {
          ...
          throw new BadRequestException(
            `Invalid URL encoding in path parameter: ${paramName}`,
          );
        }
  ...
  async executeHandler(
    ...
    if (route.pathParamSchema) {
      try {
        validatedPathParams = (await route.pathParamSchema.parseAsync(
          params.pathParams,
    ...
    if (route.queryParamSchema) {
      try {
        validatedQueryParams = (await route.queryParamSchema.parseAsync(
          params.queryParams,
    ...
    const handlerParams = {
      ...
      ...(typeof validatedBody === 'object' && validatedBody !== null
        ? this.omitReservedKeys(validatedBody)
        : {}),
      ...context,
    };
  ...
  private omitReservedKeys(value: unknown): Record<string, unknown> {
    ...
    for (const [key, entryValue] of Object.entries(value)) {
      if (this.reservedParamKeys.has(key)) {
        this.logger.warn(
          `Ignoring reserved handler parameter key "${key}" from request input`,
        );
        continue;
      }
